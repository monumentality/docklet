...
""" % (username, 10000, cookiename, '/go/'+username+'/'+clustername, '/jupyter',
    ...
    def change_cgroup_settings(self, task):
        ...
        conffile = open("/var/lib/lxc/%s/config" % configuration['lxc_name'],"r")
        content = conffile.read()
        conffile.close()

        content = content.replace("%CONTAINER_MEMORY%",str(task['allocation_mems']))
        content = content.replace("%CONTAINER_MEMORY_SW%",str(task['allocation_mems_sw']))
        content = content.replace("%CONTAINER_MEMORY_SOFT%",str(task['allocation_mems_soft']))
        content = content.replace("%CONTAINER_CPU%",str(task['allocation_cpus']))
