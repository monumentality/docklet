...
class Container(object):
    ...
    def create_container(self, lxc_name, username, setting, clustername, clusterid, containerid, hostname, ip, gateway, vlanid, image, allocation):
        ...
        try:
            ...
            def config_prepare(content):
                ...
                content = content.replace("%CONTAINER_MEMORY%",str(allocation['allocation_mems']))
#                content = content.replace("%CONTAINER_MEMORY_SW%",str(allocation['allocation_mems_sw']))
#                content = content.replace("%CONTAINER_MEMORY_SOFT%",str(allocation['allocation_mems_soft']))
                content = content.replace("%CONTAINER_CPU%",str(allocation['allocation_cpus']))
...
""" % (username, 10000, cookiename, '/go/'+username+'/'+clustername, '/jupyter',
    ...
    # when a task's allocation_type is changed, the scheduler will call change_cgroup_settings
    def change_cgroup_settings(self, allocation):
        
        logger.debug("change_cgroup_settings")
        
        conffile = open("/var/lib/lxc/%s/config" % allocation['id'],"r")
        content = conffile.read()
        conffile.close()

        new_content = ""
        for line in content.splitlines():
            if line.startswith("lxc.cgroup.memory.limit_in_bytes"):
                new_line = "lxc.cgroup.memory.limit_in_bytes = "+ str(allocation['allocation_mems']) +"M\n"
                new_content += new_line
            elif line.startswith("lxc.cgroup.cpu.shares"):
                new_line = "lxc.cgroup.cpu.shares = " + str(allocation['allocation_cpus']) +"\n"
                new_content += new_line
            else:
                new_content += (line +"\n")
...
#        logger.debug("new content: %s",new_content)
        conffile = open("/var/lib/lxc/%s/config" % allocation['id'],"w")
        conffile.write(new_content)
        conffile.close()

        command = 'sudo /bin/bash -c "echo %s > /sys/fs/cgroup/%s/lxc/%s/%s"'
        cpu_command = command % (allocation['allocation_cpus'], 'cpu', allocation['id'], 'cpu.shares')
        mems_command = command % (allocation['allocation_mems']+'M', 'memory', allocation['id'], 'memory.limit_in_bytes')
        mems_soft_command = command % (allocation['allocation_mems_soft']+'M', 'memory', allocation['id'], 'memory.soft_limit_in_bytes')
        mems_sw_command = command % (allocation['allocation_mems_sw']+'M', 'memory', allocation['id'], 'memory.memsw.limit_in_bytes')
        os.system(cpu_command)
        os.system(mems_sw_command)
        os.system(mems_command)
        os.system(mems_soft_command)
        return
